...
<input id="level" value="<%=UserInfo.getLevel()%>" />
<input value="<%=UserInfo.getName()%>" />
<input id="userId" value="<%=UserInfo.getUserId()%>" />
<input id="currentLevel" value="0" />
<%--data set--%>
<script>
  /* check if data need to update*/
  var change = document.getElementById("level").getAttribute("value");
  if (change != -1){
    var local = window.sessionStorage.getItem("level");
    if(local > change){
      document.getElementById("level").setAttribute("value",local);
    }
  }
</script>
...
<div id="level-button-container">
  ...
  <form method = "post" action ="/SaveServlet">
    <input name="level" value="<%=UserInfo.getLevel()+1000%>" hidden/>
